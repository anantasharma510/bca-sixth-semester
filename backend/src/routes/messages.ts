...
const canMessageEachOther = async (userId1: string, userId2: string): Promise<boolean> => {
  try {
...
router.get('/following/:userId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const mutualFollowers = following.filter(f => 
      f.followingId && 
      f.followingId._id && 
    ...
    // Check for blocks
    const blockedUsers = await Block.find({ blockerId: userId }).lean();
    ...
    const messageableUsers = mutualFollowers
      ...
      .map(f => ({
        ...f.followingId,
        _id: f.followingId._id.toString()
      }));
...
router.get('/conversations/:userId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.log('🔍 DB User ID:', (req as any).userId);
    ...
    const formattedConversations = conversations.map((conv: any) => ({
      ...
      otherParticipant: conv.otherParticipant ? {
        ...
        _id: conv.otherParticipant._id ? conv.otherParticipant._id.toString() : null
      } : null
    }));
...
router.get('/conversations/:userId/search', requireAuth, [
  ...
  try {
    ...
    const formattedConversations = conversations.map((conv: any) => ({
      ...
      otherParticipant: conv.otherParticipant ? {
        ...conv.otherParticipant,
        _id: conv.otherParticipant._id.toString()
      } : null
    }));
...
router.get('/unread-count/:userId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
router.post('/conversations', requireAuth, [
  ...
  try {
    ...
    const canMessage = await canMessageEachOther(userId, participantId);
    if (!canMessage) {
    ...
    let conversation = await Conversation.findOne({ participants: { $all: [userId, participantId] } });
    ...
    const conversationWithDetails = await Conversation.findById(conversation._id)
    ...
    const otherParticipant = conversationWithDetails.participants.find(
    ...
    const formattedResult = {
      ...
      _id: conversationWithDetails._id.toString(),
      participants: conversationWithDetails.participants.map((participant: any) => ({
        ...participant,
...
router.put('/conversations/:conversationId/read', requireAuth, async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/messages/:conversationId', requireAuth, [
  ...
  try {
    ...
    const transformedMessages = messages.map(message => {
      ...
      const senderId = messageObj.senderId as any;
...
router.get('/messages/:conversationId/search', requireAuth, [
  ...
  try {
    ...
    const messages = await Message.find({ 
      conversationId, 
      deletedAt: { $exists: false },
      content: { $regex: sanitizeInput(q as string), $options: 'i' }
    })
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
...
router.post('/messages', requireAuth, [
  ...
  try {
    ...
    await message.populate('senderId', 'username firstName lastName profileImageUrl');
    ...
    const transformedMessage = {
      ...
      senderId_details: senderIdDetails
    };
...
router.post('/upload', requireAuth, (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const result = await uploadToCloudinary(req.file.buffer, 'messages', isVideo ? 'video' : 'image') as any;
