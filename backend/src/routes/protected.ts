...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const users = await User.find({}, '_id username email firstName lastName profileImageUrl role status createdAt lastActivityAt followerCount followingCount postCount');
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const result = await uploadToCloudinary(req.file.buffer, `profiles/${type}`);
...
router.get('/users/search', requireAuth, asyncHandler(async (req, res, next) => {
  ...
  const blockedUsers = await Block.find({ blockerId: currentUserId }).select('blockedId').lean();
  const blockedIds = blockedUsers.map((b: any) => b.blockedId);

  // Get users who blocked current user
  const blockedByUsers = await Block.find({ blockedId: currentUserId }).select('blockerId').lean();
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const Block = (await import('../models/block.model.js')).Block;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const trendingHashtags = await getTrendingHashtags({ days: 7, limit: 5 });
    // Recent signups (last 7 days)
    const since = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const recentSignups = await User.find({ createdAt: { $gte: since } })
...
// Admin-only: Get AI config (free tier + plans)
router.get(
  '/admin/ai-config',
  requireAdmin,
  asyncHandler(async (req: Request, res: Response) => {
...
router.post(
  ...
  requireAdmin,
  asyncHandler(async (req: Request, res: Response) => {
    const { freeMonthlyOutfits, plans } = req.body || {};
    ...
    if (typeof freeMonthlyOutfits === 'number' && freeMonthlyOutfits >= 0) {
      cfg.freeMonthlyOutfits = freeMonthlyOutfits;
    }

    if (Array.isArray(plans)) {
      cfg.plans = plans.map((p: any) => ({
        key: p.key,
        name: p.name,
        monthlyOutfitLimit: p.monthlyOutfitLimit ?? null,
        amountCents: typeof p.amountCents === 'number' ? p.amountCents : null,
        currency: p.currency || 'usd',
        // stripePriceId is managed by the backend when creating Stripe prices.
...
// Admin-only: list subscription transactions
router.get(
...
// Authenticated user: own subscription & transaction history
router.get(
  '/subscription/me',
  requireAuth,
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [subscription, transactions] = await Promise.all([
      ...
      SubscriptionTransaction.find({ userId }).sort({ createdAt: -1 }).limit(50),
    ]);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const authorUsername = (req.query.authorUsername as string || '').trim();
    const from = req.query.from ? new Date(req.query.from as string) : undefined;
    const to = req.query.to ? new Date(req.query.to as string) : undefined;
    const hasMedia = req.query.hasMedia === 'true';
    const hashtag = (req.query.hashtag as string || '').trim();
    const query: any = {};
    if (search) {
    ...
    if (authorUsername) {
      ...
      else query.author = '__none__'; // will return empty
    }
    if (from || to) {
      query.createdAt = {};
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const dbUser = await User.findById(clerkUser.sub);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const originalPost = (req.query.originalPost as string || '').trim();
    const from = req.query.from ? new Date(req.query.from as string) : undefined;
    const to = req.query.to ? new Date(req.query.to as string) : undefined;
    const query: any = {};
    if (search) {
    ...
    const total = await Repost.countDocuments(query);
...
router.get('/maintenance', (req, res) => {
  try {
    Maintenance.findOne().sort({ updatedAt: -1 }).then(doc => {
      res.json({ enabled: doc?.enabled || false, message: doc?.message || '', data: doc?.data || {} });
    }).catch(() => {
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (status) {
      ...
      if ((status === 'resolved' || status === 'closed') && !ticket.resolvedAt) {
        ...
        updates.resolvedBy = (req as any).userId;
      }
    }
    ...
    console.log(`✅ Admin ${(req as any).userId} updated support ticket ${ticketId}`);
