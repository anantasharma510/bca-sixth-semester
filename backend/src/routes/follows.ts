...
router.post('/:userId/follow', [
  ...
  try {
    ...
    if (existingFollow) {
      ...
      const [targetUserFollowersCount, targetUserFollowingCount, followerFollowersCount, followerFollowingCount] = await Promise.all([
      ...
      const isFollowedBy = await Follow.findOne({ followerId: userId, followingId: followerId });
...
router.get('/:userId/followed-by', [
  ...
  try {
    ...
    const follow = await Follow.findOne({ followerId: userId, followingId: followerId });
...
router.get('/suggestions', [
  ...
  try {
    ...
    const followingIds = await Follow.find({ followerId: currentUserId })
