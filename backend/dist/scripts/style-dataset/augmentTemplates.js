...
import { createDefaultForm, createDefaultProfile, writeSamplesToJsonl, ensureSampleValidity, } from './utils';
...
function applyVariations(items) {
    return items.map((item) => {
        ...
        words[0] = colorPrefix;
        return Object.assign(Object.assign({}, item), { query: words.join(' '), price: Math.round(item.price * (0.9 + Math.random() * 0.2)) });
    });
}
function buildSample(template, iteration) {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const outputPath = process.env.STYLE_TEMPLATE_OUTPUT ||
