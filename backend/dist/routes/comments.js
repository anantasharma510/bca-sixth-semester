...
const router = Router();
// Update a comment
router.put('/:commentId', requireAuth, (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Update the comment
        const updatedComment = yield Comment.findByIdAndUpdate(commentId, { content: content.trim() }, { new: true }).populate('author', 'username firstName lastName profileImageUrl');
...
// Delete a comment
router.delete('/:commentId', requireAuth, (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.userId;
        const { commentId } = req.params;
        // Find the comment and check ownership
        ...
        yield Like.deleteMany({
            $or: [
                { comment: commentId },
                { comment: { $in: yield Comment.find({ parentComment: commentId }).select('_id').lean() } }
            ]
        });
        // Delete the comment
...
router.post('/:commentId/like', requireAuth, (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.userId;
        const { commentId } = req.params;
        const comment = yield Comment.findById(commentId);
        ...
        if (existingLike) {
            ...
            const updatedComment = yield Comment.findByIdAndUpdate(commentId, { $inc: { likeCount: -1 } }, { new: true });
            // Emit unlike event via Socket.IO
            io.to(`post_${comment.post.toString()}`).emit('commentLiked', {
                commentId,
                liked: false,
                likeCount: (updatedComment === null || updatedComment === void 0 ? void 0 : updatedComment.likeCount) || 0
            });
            res.json({ liked: false, likeCount: (updatedComment === null || updatedComment === void 0 ? void 0 : updatedComment.likeCount) || 0 });
        }
        else {
            // Like
            ...
            res.json({ liked: true, likeCount: (updatedComment === null || updatedComment === void 0 ? void 0 : updatedComment.likeCount) || 0 });
        }
    }
    catch (error) {
...
router.post('/:commentId/replies', requireAuth, (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.userId;
        const { commentId } = req.params;
        const { content } = req.body;
        ...
        const parentComment = yield Comment.findById(commentId);
        if (!parentComment) {
        ...
        // Create the reply
        const reply = yield Comment.create({
            author: userId,
        ...
        yield Comment.findByIdAndUpdate(commentId, { $push: { replies: reply._id } });
        ...
        yield reply.populate('author', 'username firstName lastName profileImageUrl');
        ...
        const replyWithStatus = Object.assign(Object.assign({}, reply.toObject()), { isLiked: false, replyCount: 0 });
...
router.get('/:commentId/replies', requireAuth, (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.userId;
        const { commentId } = req.params;
        ...
        const replies = yield Comment.find({ parentComment: commentId })
        ...
        const repliesWithLikeStatus = replies.map((reply) => {
            ...
            return Object.assign(Object.assign({}, reply), { author: authorData, isLiked: likeMap.has(reply._id.toString()), replyCount: replyCountMap.get(reply._id.toString()) || 0 });
        });
        // Get total count for pagination
        const totalReplies = yield Comment.countDocuments({ parentComment: commentId });
        const response = {
