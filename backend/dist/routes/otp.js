...
const router = Router();
const logOtpForDebug = (email, code, type) => {
...
router.post('/send-signup', [
    ...
    try {
        ...
        const existingUser = yield User.findOne({ email: normalizedEmail });
        if (existingUser) {
            return res.status(400).json({ error: 'User with this email already exists' });
        }
        // Delete any existing unverified OTPs for this email and type
        yield OTP.deleteMany({
        ...
        let emailSent = true;
        try {
            yield sendOTPEmail(normalizedEmail, code, 'signup');
            ...
            if (process.env.NODE_ENV === 'production') {
                yield OTP.findByIdAndDelete(otp._id);
...
router.post('/send-password-reset', [
    ...
    try {
        ...
        const user = yield User.findOne({ email: normalizedEmail });
        if (!user) {
        ...
        let emailSent = true;
        try {
            yield sendOTPEmail(normalizedEmail, code, 'password-reset');
